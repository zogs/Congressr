...
<div class="w70pc fleft">
	<?php if($type=='resume'): ?>
		<form class="form form-center " action="<?php echo Router::url('admin/articles/view/resume/'.$resume->id);?>" method="POST">
			...
			<?php echo $this->Form->input('title','Titre de votre résumé',array('placeholder'=>'Titre du résumé','type'=>'textarea','rows'=>2,'value'=>$resume->title)) ;?>
			<?php echo $this->Form->input('text','Contenu de votre résumé',array('placeholder'=>'Contenu du résumé','type'=>'textarea','rows'=>20,'value'=>$resume->text)) ;?>
...
<div class="w30pc fright">
	...
	<form class="form form-row" action="<?php echo Router::url('admin/articles/assign/'.$type.'/'.((isset($$type->resume_id)? $$type->resume_id : $$type->id)));?>" method="POST">

	<?php echo $this->Form->input('token','hidden',array('value'=>Session::token())); ?>
	<?php echo $this->Form->select('reviewer','Envoyer une demande de review',$reviewers,array('style'=>'width:auto;padding-left:0;')) ;?>
	<?php echo $this->Form->input('submit','',array('type'=>'submit','class'=>'btn btn-primary','value'=>'Envoyer','style'=>'margin:0')); ?>
	...
	<form class="form form-row" action="<?php echo Router::url('admin/articles/decision/'.$type.'/'.((isset($$type->resume_id)? $$type->resume_id : $$type->id)));?>" method="POST">
		<?php echo $this->Form->select('decision','Decision',array('accepted'=>'accepted','refused'=>'refused','pending'=>'pending','reviewed'=>'reviewed'),array('style'=>'width:auto;padding-left:0;')); ?>
		<?php echo $this->Form->input('token','hidden',array('value'=>Session::token())); ?>
		<?php echo $this->Form->input('submit','',array('type'=>'submit','class'=>'btn btn-primary','value'=>'Envoyer','style'=>'margin:0')); ?>
